...
if(@$_REQUEST['log_out'])
{
	// session start allows you to login 
	//@ gives an error logout and delete session cookie
	session_destroy();
...
if($_SESSION['login'] == 'y')
{
   ...
   while($row = $myData->fetch_assoc())
   {
	...
	if($_POST['submitted'])
	{
		//store cleaned POST data into vars or variable
...
{	//we can only update the page, if we login. if not login, it bounce the user back to the index page
